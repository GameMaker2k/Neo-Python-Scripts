...
def imageaffinematrixconcat(matrix1, matrix2):
    ...
    """
    # In GD, matrices are 6-element lists
...
def imageavif(image_res, filename=None):
    ...
    """
    # Pillow may not support AVIF depending on the version
    if filename:
...
def imagefilter(image_res, filtertype, *args):
    ...
    if filtertype == IMG_FILTER_NEGATE:
        ...
        image_res.image = image_res.image.filter(ImageFilter.RankFilter(size=3, rank=0))
...
def imagefilledpolygon(image_res, points, num_points, color_index):
    """
    Draw a filled polygon.
    """
    color = image_res.get_color(color_index)
    image_res.draw.polygon(points, fill=color)
...
def imagejpeg(image_res, filename=None, quality=75):
    """
    Output image to a JPEG file.
    """
    if filename:
        image_res.image.save(filename, "JPEG", quality=quality)
    else:
        image_res.image.save(sys.stdout, "JPEG", quality=quality)
...
def imagesavealpha(image_res, saveflag):
    """
    Set whether to save full alpha channel information when saving images.
    """
