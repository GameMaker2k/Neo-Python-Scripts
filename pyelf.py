...
def get_elf_header(infile):
 ...
 if(elf_file_info['e_phnum'][1]<=0):
  ...
  numprohead = int(str(elf_file_info['e_phnum'][0])+str(elf_file_info['e_phnum'][1]));
 if(elf_file_info['e_phentsize'][1]<=0):
  proheadersize = elf_file_info['e_phentsize'][0];
 else:
  proheadersize = int(str(elf_file_info['e_phentsize'][0])+str(elf_file_info['e_phentsize'][1]));
 ...
 while(cintnumprohead < numprohead):
  ...
  phsum = phend - phstart;
  cintnumprohead += 1;
 elf_file_info.update({'e_file_info': infile+": ELF "+elf_file_info['e_bit']+" "+elf_file_info['ei_significantbyte']+" "+elf_file_info['e_objectmeaning']+" "+elf_file_info['e_architecture']+", version 1 ("+elf_file_info['ei_osname']+")"});
